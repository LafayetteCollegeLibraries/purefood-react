...
//var tileSource = new OpenSeadragon.DjatokaTileSource('/adore-djatoka/resolver', 'http://purefood.stage.lafayette.edu/img/cso_1880_export.jp2', tileSourceSettings);
var tileSources = new Array();
tileSources.push(new OpenSeadragon.DjatokaTileSource('/adore-djatoka/resolver', 'http://purefood.stage.lafayette.edu/img/cso_1880_export.jp2', tileSourceSettings));
tileSources.push(new OpenSeadragon.DjatokaTileSource('/adore-djatoka/resolver', 'http://purefood.stage.lafayette.edu/img/cso_1880_export.jp2', tileSourceSettings));
tileSources.push(new OpenSeadragon.DjatokaTileSource('/adore-djatoka/resolver', 'http://purefood.stage.lafayette.edu/img/cso_1880_export.jp2', tileSourceSettings));
...
var config = {
    ...
    'sequenceMode': true,
    'zoomInButton':   "map-zoom-slider-in",
    'zoomOutButton':  "map-zoom-slider-out",
    'showFullPageControl': false
};

var viewer = OpenSeadragon(config);
$(document).data('viewer', viewer);
...
viewer.addHandler("open", function (data) {
	$('#map-zoom-slider-in, #map-zoom-slider-out, #map-zoom-slider-home').removeAttr('style');
	console.log( $('#map-zoom-slider-in #map-zoom-slider-out #map-zoom-slider-home') );
    });

/**
 * Integration for the time slider
 *
 */
var TIME_STEPS_MAX = 3;
var tickValues = Array.apply(null, Array(3)).map(function (_, i) { return i; });
var tickLabels = Array.apply(null, Array(3)).map(function (_, i) { return (i*10 + 1880).toString(); });
...
var timeControl = $("#input-time-slider").slider({ id: 'time-slider',
						   ...
						   tooltip: 'hide' }).on('change', function(e) {
							   ...
							   viewer.goToPage(e.value.newValue);
						       });
